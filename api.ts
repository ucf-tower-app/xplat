...
export function getAuthUser() { return auth.currentUser }

export async function sendAuthEmail() {
    if (auth.currentUser != null) {
        if (auth.currentUser.emailVerified) console.log("Already verified, not sending email.")
        else sendEmailVerification(auth.currentUser)
            .then(() => {
